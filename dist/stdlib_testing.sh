...
builtin declare -- STDLIB_TESTING_PARAMETRIZE_SCENARIO_NAME=""
...
builtin declare -- STDLIB_TESTING_THEME_DEBUG_FIXTURE="GREY"
builtin declare -- STDLIB_TESTING_THEME_LOAD="GREY"
builtin declare -- STDLIB_TESTING_THEME_PARAMETRIZE_HIGHLIGHT="LIGHT_BLUE"
builtin declare -- STDLIB_TESTING_THEME_PARAMETRIZE_ORIGINAL_TEST_NAMES="GREY"
...
builtin declare -a "__STDLIB_TESTING_MOCK_RESTRICTED_ATTRIBUTES=([0]='builtin' [1]='case' [2]='do' [3]='done' [4]='elif' [5]='else' [6]='esac' [7]='fi' [8]='for' [9]='if' [10]='while')"
builtin declare -a "__STDLIB_TESTING_MOCK_SEQUENCE_ARRAY=()"
builtin declare -- __STDLIB_TESTING_MOCK_SEQUENCE_FILENAME=""
builtin declare -- __STDLIB_TESTING_MOCK_SEQUENCE_LOCK_NAME="__stdlib_testing_internal__mock_sequence_lock"
builtin declare -- __STDLIB_TESTING_MOCK_SEQUENCE_TRACKING_BOOLEAN="0"
builtin declare -a "__STDLIB_TESTING_PARAMETRIZE_GENERATED_FUNCTIONS_ARRAY=()"
builtin declare -- __STDLIB_TESTING_PROTECT_PREFIX=""
...
# @exitcode 0 If the mock's call was retrieved successfully.
...
# @noargs
# @exitcode 0 If all the mock's calls were retrieved successfully.
# @exitcode 127 If the wrong number of arguments were provided.
...
# @arg $1 string The arg string to compare against the mock's call history.
# @exitcode 0 If the mock was called at least once with the specified arguments.
# @exitcode 1 If the mock was never called with the specified arguments.
# @exitcode 126 If an invalid argument has been provided.
# @exitcode 127 If the wrong number of arguments were provided.
...
# @exitcode 0 If the call at the specified index in the mock's call history matches the given arg string.
# @exitcode 1 If the call at the specified index in the mock's call history does not match the given arg string.
# @exitcode 126 If an invalid argument has been provided.
# @exitcode 127 If the wrong number of arguments were provided.
# @stderr The error message if the assertion fails.
...
# @arg $1 string The arg string to compare against the mock's call history.
# @exitcode 0 If the mock was called exactly once with the given arg string.
# @exitcode 1 If the mock was not called exactly once with the given arg string.
# @exitcode 126 If an invalid argument has been provided.
# @exitcode 127 If the wrong number of arguments were provided.
...
# @exitcode 0 If the mock was called a number of times that matches the specified count.
# @exitcode 1 If the mock was called a number of times that does not match the specified count.
# @exitcode 126 If an invalid argument has been provided.
# @exitcode 127 If the wrong number of arguments were provided.
# @stderr The error message if the assertion fails.
...
# @noargs
# @exitcode 0 If the mock was not called.
# @exitcode 1 If the mock was called any number of times.
# @exitcode 127 If the wrong number of arguments were provided.
# @stderr The error message if the assertion fails.
...
_mock.arg_string.make.from_string ()
{
    ...
    _testing.__protected stdlib.fn.keyword.consume _mock_placeholder STDLIB_FIELD_DELIMITER_ENCODE_CHAR '';
    STDLIB_KW_SOURCE_VAR="_mock_delimiter" _testing.__protected stdlib.fn.keyword.assert.is_valid_with "$(_testing.__protected_name stdlib.string.assert.not_empty)" STDLIB_FIELD_DELIMITER || builtin return 125;
    STDLIB_KW_SOURCE_VAR="_mock_placeholder" _testing.__protected stdlib.fn.keyword.assert.is_valid_with "$(_testing.__protected_name stdlib.string.assert.is_char)" STDLIB_FIELD_DELIMITER_ENCODE_CHAR || builtin return 125;
    if [[ -n "${2}" ]]; then
        _mock_arg_string_args+=("${2}");
    fi;
    STDLIB_FIELD_DELIMITER_ENCODE_CHAR="${_mock_placeholder}" _testing.__protected stdlib.array.make.from_string _mock_args_array "${_mock_delimiter}" "${1}" || builtin return "$?";
...
{
    builtin local stdlib_library_prefix="${__STDLIB_TESTING_PROTECT_PREFIX:-"stdlib"}";
...
assert_array_length ()
{
    ...
    assert_equals "${_stdlib_expected_length}" "${#_stdlib_indirect_array[*]}" || {
